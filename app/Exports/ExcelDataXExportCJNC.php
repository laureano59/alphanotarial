...
use App\Notascredito_cajarapida_interfaz_view;
use App\Conceptos_cajarapida;
use App\Cuentas_adicionales;
use App\Notas_credito_cajarapida;
use Carbon\Carbon;

class ExcelDataXExportCJNC implements FromCollection,WithHeadings
{
    private $fecha1;
    private $fecha2;
    ...
    */
    ...
    public function collection()
    {
        ...
        $Facturascajarapida = Notascredito_cajarapida_interfaz_view::select([
            'prefijo',
            'id_fact',
            'usuario_fact',
            'fecha_fact',
        ...
        $NUMFOLDER_CR_NC = Cuentas_adicionales::
             where('id_cue', 24)->value('cuenta_contab');      

       
        $TIPODOCUMENTO_C_RAPIDA_NC = Cuentas_adicionales::
             where('id_cue', 34)->value('cuenta_contab');

        $DOCUMENTO_CRUCE_CR_NC = Cuentas_adicionales::
             where('id_cue', 36)->value('cuenta_contab');
        ...
        foreach ($Facturascajarapida as $Fcjr) {
            ...
            foreach ($detalle_Fcjr as $key => $detfes) {
                ...
                if ($Fcjr->credito_fact == true){
                    ...
                    $this->cargar_informe($data, $CodigoEmpresa, $CU, $TIPOFOLDER, $NUMFOLDER_CR_NC, $Fcjr->fecha_ncf, $cuenta_cont, $Fcjr->a_nombre_de, $TIPODOCUMENTO_C_RAPIDA_NC, $Fcjr->id_ncf, 'D', $detfes['subtotal'], '', '', '', '', '', '', '', $Fecha_credito,  $Fcjr->a_nombre_de, $Fcjr->nombre_cli, $Fcjr->id_tipoident, $Fcjr->direccion_cli, $Fcjr->telefono_cli, '', '', $cod_ciud, '', '', 'N', 'N', 'S', 'N', 'N', 'N', 'N', 'N', '30', '', '', '01', $i, $j);

                }elseif ($Fcjr->credito_fact == false){
                    $this->cargar_informe($data, $CodigoEmpresa, $CU, $TIPOFOLDER, $NUMFOLDER_CR_NC, $Fcjr->fecha_ncf, $cuenta_cont, $Fcjr->a_nombre_de, $TIPODOCUMENTO_C_RAPIDA_NC, $Fcjr->id_ncf, 'D', $detfes['subtotal'], '', '', '', '', '', '', '', $Fcjr->fecha_ncf,  $Fcjr->a_nombre_de, $Fcjr->nombre_cli, $Fcjr->id_tipoident, $Fcjr->direccion_cli, $Fcjr->telefono_cli, '', '', $cod_ciud, '', '', 'N', 'N', 'S', 'N', 'N', 'N', 'N', 'N', '01', '', '', '01', $i, $j);
            ...
            /***************************************************/
             ...
             where('id_cue', 30)->value('cuenta_contab');
            ...
            $cuenta_deduc_ica = Cuentas_adicionales::
             ...
             if($Fcjr->total_iva > 0){
                ...
                if ($Fcjr->credito_fact == true){
                    $this->cargar_informe($data, $CodigoEmpresa, $CU, $TIPOFOLDER, $NUMFOLDER_CR_NC, $Fcjr->fecha_ncf, $cuenta_cont_iva_cr, $Fcjr->a_nombre_de, $TIPODOCUMENTO_C_RAPIDA_NC, $Fcjr->id_ncf, 'D', round($Fcjr->total_iva), '', '', '', '', '', '', '', $Fecha_credito, $Fcjr->a_nombre_de, $Fcjr->nombre_cli, $Fcjr->id_tipoident, $Fcjr->direccion_cli, $Fcjr->telefono_cli, '', '', $cod_ciud, '', '', 'N', 'N', 'S', 'N', 'N', 'N', 'N', 'N', '30', '', '', '01', $i, $j);

                }elseif ($Fcjr->credito_fact == false){
                    $this->cargar_informe($data, $CodigoEmpresa, $CU, $TIPOFOLDER, $NUMFOLDER_CR_NC, $Fcjr->fecha_ncf, $cuenta_cont_iva_cr, $Fcjr->a_nombre_de, $TIPODOCUMENTO_C_RAPIDA_NC, $Fcjr->id_ncf, 'D', round($Fcjr->total_iva), '', '', '', '', '', '', '', $Fcjr->fecha_ncf, $Fcjr->a_nombre_de, $Fcjr->nombre_cli, $Fcjr->id_tipoident, $Fcjr->direccion_cli, $Fcjr->telefono_cli, '', '', $cod_ciud, '', '', 'N', 'N', 'S', 'N', 'N', 'N', 'N', 'N', '01', '', '', '01', $i, $j);
                }
             }
        ...
        if($Fcjr->deduccion_reteiva > 0){
            ...
            if ($Fcjr->credito_fact == true){
                $this->cargar_informe($data, $CodigoEmpresa, $CU, $TIPOFOLDER, $NUMFOLDER_CR_NC, $Fcjr->fecha_ncf, $cuenta_deduc_iva, $Fcjr->a_nombre_de, $TIPODOCUMENTO_C_RAPIDA_NC, $Fcjr->id_ncf, 'C', round($Fcjr->deduccion_reteiva), '', '', '', '', '', '', '', $Fecha_credito, $Fcjr->a_nombre_de, $Fcjr->nombre_cli, $Fcjr->id_tipoident, $Fcjr->direccion_cli, $Fcjr->telefono_cli, '', '', $cod_ciud, '', '', 'N', 'N', 'S', 'N', 'N', 'N', 'N', 'N', '30', '', '', '01', $i, $j);

            }elseif ($Fcjr->credito_fact == false){
                $this->cargar_informe($data, $CodigoEmpresa, $CU, $TIPOFOLDER, $NUMFOLDER_CR_NC, $Fcjr->fecha_ncf, $cuenta_deduc_iva, $Fcjr->a_nombre_de, $TIPODOCUMENTO_C_RAPIDA_NC, $Fcjr->id_ncf, 'C', round($Fcjr->deduccion_reteiva), '', '', '', '', '', '', '', $Fcjr->fecha_ncf, $Fcjr->a_nombre_de, $Fcjr->nombre_cli, $Fcjr->id_tipoident, $Fcjr->direccion_cli, $Fcjr->telefono_cli, '', '', $cod_ciud, '', '', 'N', 'N', 'S', 'N', 'N', 'N', 'N', 'N', '01', '', '', '01', $i, $j);
        ...
        if($Fcjr->deduccion_reteica > 0){
            ...
            if ($Fcjr->credito_fact == true){
                $this->cargar_informe($data, $CodigoEmpresa, $CU, $TIPOFOLDER, $NUMFOLDER_CR_NC, $Fcjr->fecha_ncf, $cuenta_deduc_ica, $Fcjr->a_nombre_de, $TIPODOCUMENTO_C_RAPIDA_NC, $Fcjr->id_ncf, 'C', round($Fcjr->deduccion_reteica), '', '', '', '', '', '', '', $Fecha_credito, $Fcjr->a_nombre_de, $Fcjr->nombre_cli, $Fcjr->id_tipoident, $Fcjr->direccion_cli, $Fcjr->telefono_cli, '', '', $cod_ciud, '', '', 'N', 'N', 'S', 'N', 'N', 'N', 'N', 'N', '30', '', '', '01', $i, $j);

            }elseif ($Fcjr->credito_fact == false){
                $this->cargar_informe($data, $CodigoEmpresa, $CU, $TIPOFOLDER, $NUMFOLDER_CR_NC, $Fcjr->fecha_ncf, $cuenta_deduc_ica, $Fcjr->a_nombre_de, $TIPODOCUMENTO_C_RAPIDA_NC, $Fcjr->id_ncf, 'C', round($Fcjr->deduccion_reteica), '', '', '', '', '', '', '', $Fcjr->fecha_ncf, $Fcjr->a_nombre_de, $Fcjr->nombre_cli, $Fcjr->id_tipoident, $Fcjr->direccion_cli, $Fcjr->telefono_cli, '', '', $cod_ciud, '', '', 'N', 'N', 'S', 'N', 'N', 'N', 'N', 'N', '01', '', '', '01', $i, $j);
            }
        ...
        if($Fcjr->deduccion_retertf > 0){
            ...
            if ($Fcjr->credito_fact == true){
                $this->cargar_informe($data, $CodigoEmpresa, $CU, $TIPOFOLDER, $NUMFOLDER_CR_NC, $Fcjr->fecha_ncf, $cuenta_deduc_rtf, $Fcjr->a_nombre_de, $TIPODOCUMENTO_C_RAPIDA_NC, $Fcjr->id_ncf, 'C', round($Fcjr->deduccion_retertf), '', '', '', '', '', '', '', $Fecha_credito, $Fcjr->a_nombre_de, $Fcjr->nombre_cli, $Fcjr->id_tipoident, $Fcjr->direccion_cli, $Fcjr->telefono_cli, '', '', $cod_ciud, '', '', 'N', 'N', 'S', 'N', 'N', 'N', 'N', 'N', '30', '', '', '01', $i, $j);

            }elseif ($Fcjr->credito_fact == false){
                $this->cargar_informe($data, $CodigoEmpresa, $CU, $TIPOFOLDER, $NUMFOLDER_CR_NC, $Fcjr->fecha_ncf, $cuenta_deduc_rtf, $Fcjr->a_nombre_de, $TIPODOCUMENTO_C_RAPIDA_NC, $Fcjr->id_ncf, 'C', round($Fcjr->deduccion_retertf), '', '', '', '', '', '', '', $Fcjr->fecha_ncf, $Fcjr->a_nombre_de, $Fcjr->nombre_cli, $Fcjr->id_tipoident, $Fcjr->direccion_cli, $Fcjr->telefono_cli, '', '', $cod_ciud, '', '', 'N', 'N', 'S', 'N', 'N', 'N', 'N', 'N', '01', '', '', '01', $i, $j);
        ...
        if ($Fcjr->credito_fact == true){ //Si es a credito
            $j = $j + 1;
            $this->cargar_informe($data, $CodigoEmpresa, $CU, $TIPOFOLDER, $NUMFOLDER_CR_NC, $Fcjr->fecha_ncf, $cuenta_a_credito, $Fcjr->a_nombre_de, $TIPODOCUMENTO_C_RAPIDA_NC, $Fcjr->id_ncf, 'C', round($Fcjr->total_fact), '', '', '', '', $DOCUMENTO_CRUCE_CR_NC,  $Fcjr->id_fact, '', $Fecha_credito,  $Fcjr->a_nombre_de, $Fcjr->nombre_cli, $Fcjr->id_tipoident, $Fcjr->direccion_cli, $Fcjr->telefono_cli, '', '', $cod_ciud, '', '', 'N', 'N', 'S', 'N', 'N', 'N', 'N', 'N', '30', '', '', '01', $i, $j);
            ...
            if($total_medios_pago > 0){
                if( $Fcjr->efectivo > 0){                    
                    ...
                    $this->cargar_informe($data, $CodigoEmpresa, $CU, $TIPOFOLDER, $NUMFOLDER_CR_NC, $Fcjr->fecha_ncf, $cuenta_contado, $Fcjr->a_nombre_de, $TIPODOCUMENTO_C_RAPIDA_NC, $Fcjr->id_ncf, 'C', round($Fcjr->efectivo), '', '', '', '', $DOCUMENTO_CRUCE_CR_NC, $Fcjr->id_fact, '', $Fcjr->fecha_ncf,  $Fcjr->a_nombre_de, $Fcjr->nombre_cli, $Fcjr->id_tipoident, $Fcjr->direccion_cli, $Fcjr->telefono_cli, '', '', $cod_ciud, '', '', 'N', 'N', 'S', 'N', 'N', 'N', 'N', 'N', '01', '', '', '01', $i, $j);

                     $j = $j + 1; //Para la suma de otros medios depago

                    $this->cargar_informe($data, $CodigoEmpresa, $CU, $TIPOFOLDER, $NUMFOLDER_CR_NC, $Fcjr->fecha_ncf, $cuenta_banco, $Fcjr->a_nombre_de, $TIPODOCUMENTO_C_RAPIDA_NC, $Fcjr->id_ncf, 'C', round($total_medios_pago), '', '', '', '', $DOCUMENTO_CRUCE_CR_NC, $Fcjr->id_fact, '', $Fcjr->fecha_ncf,  $Fcjr->a_nombre_de, $Fcjr->nombre_cli, $Fcjr->id_tipoident, $Fcjr->direccion_cli, $Fcjr->telefono_cli, '', '', $cod_ciud, '', '', 'N', 'N', 'S', 'N', 'N', 'N', 'N', 'N', '01', '', '', '01', $i, $j);
                }else{
                ...
                $this->cargar_informe($data, $CodigoEmpresa, $CU, $TIPOFOLDER, $NUMFOLDER_CR_NC, $Fcjr->fecha_ncf, $cuenta_contado, $Fcjr->a_nombre_de, $TIPODOCUMENTO_C_RAPIDA_NC, $Fcjr->id_ncf, 'C', round($Fcjr->total_fact), '', '', '', '', $DOCUMENTO_CRUCE_CR_NC, $Fcjr->id_fact, '', $Fcjr->fecha_ncf,  $Fcjr->a_nombre_de, $Fcjr->nombre_cli, $Fcjr->id_tipoident, $Fcjr->direccion_cli, $Fcjr->telefono_cli, '', '', $cod_ciud, '', '', 'N', 'N', 'S', 'N', 'N', 'N', 'N', 'N', '01', '', '', '01', $i, $j);
