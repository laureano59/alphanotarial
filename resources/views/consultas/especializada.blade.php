                    ...
                    if(response.status == 'success') {
                        ...
                        if(response.data.facturas.length > 0) {
                            ...
                            response.data.facturas.forEach(function(f) {
                                ...
                                var nombreCli = f.a_nombre_de; // Identificación que ya viene en la factura
                                var nombreReal = '';
                                if (f.empresa && f.empresa.trim() != '') {
                                    nombreReal = f.empresa;
                                } else if (f.pmer_nombrecli || f.pmer_apellidocli) {
                                    nombreReal = (f.pmer_nombrecli || '') + ' ' + (f.pmer_apellidocli || '');
                                }
                                
                                if (nombreReal.trim() != '') {
                                    nombreCli += ' - ' + nombreReal.trim();
                                } else if (f.cliente_nombre && f.cliente_nombre != f.a_nombre_de) {
                        ...
                        if(response.data.actas.length > 0) {
                            ...
                            response.data.actas.forEach(function(a) {
                                ...
                                $("#tabla_actas tbody").append('<tr '+(a.saldo > 1 ? 'class="warning"' : '')+'>' +
                                    ...
                                    '<td>'+(a.anulada ? '<span class="label label-danger">ANULADA</span>' : '<span class="label class="label label-success">ACTIVA</span>')+'</td>' +
